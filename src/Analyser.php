...
final class Analyser
{
    /**
     * @readonly
     * @var \TomasVotruba\Lines\MetricsCollector
     */
            ...
            switch ($token) {
                ...
                case \T_TRAIT:
                    ...
                    if ($token === \T_TRAIT) {
                        ...
                        if ($classModifierToken && $tokens[$classModifierToken][0] === \T_ABSTRACT) {
                            $this->metricsCollector->incrementAbstractClasses();
                        } elseif ($classModifierToken && $tokens[$classModifierToken][0] === \T_FINAL) {
                ...
                case \T_CONST:
                    ...
                    if ($possibleScopeToken && \in_array($tokens[$possibleScopeToken][0], [\T_PRIVATE, \T_PROTECTED], \true)) {
    ...
    private function getPreviousNonWhitespaceNonCommentTokenPos(array $tokens, int $start)
    {
        ...
        if (isset($tokens[$previousTokenIndex])) {
            if (\in_array($tokens[$previousTokenIndex][0], [\T_WHITESPACE, \T_COMMENT, \T_DOC_COMMENT], \true)) {
